...
contract EchidnaTemplate {
    ...
    function add_sub_inverse_operations(int128 x, int128 y) public {
        emit Debug(x,y);
        int128 x_y = add(x,y);
        int128 result = sub(x_y,y);
        emit Debug(result,x_y);
        assert(result == x);
    }
    ...
    function div_test_not_commutative(int128 x, int128 y) public {
        // 1. This is a precondition. Either use the precondition or the postcondition workaround/solution
